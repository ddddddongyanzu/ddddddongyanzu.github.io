...
> 这个稍微会涉及到计算机组成原理了，不了解的同学也没有关系，以后会慢慢理解这个概念的。

从前面的枯涩描述里，换成需要大家了解的信息是：
...
> 好的命名是一个非常优秀的编程素养，如果觉得自己英文不OK，你可以使用**有道词典**进行翻译，切记尽可能用英文单词命名变量哦 另外，这个变量名的首字母一般是小写的哦，不要纠结为什么，Java 习惯这样命名。
...
这里有个概念就好，基本上你根据文字颜色就可以区分出来的。如果写错了，控制台也会有错误提示的哦
...
```java
String say = "Hello 优课达";
```

如上面的代码，赋值语句就是在变量名后面加上 `= 数据`，如果数据是字符类型，还需要使用 `""` 包围住字符串，如上面的 `"Hello 优课达"`

##### 重复赋值

除了上面的赋值方式外，我们还可以对变量重新赋值， 我们通过代码演示来理解这句话

#### 1.3 数据类型

通过前面的学习，其实我们接触了两个数据类型：字符串、整数
...
注意 long 类型赋值的时候要在数字最后面跟上一个小写的 `l`(注意是字母 L 不要看错啦),比如
...
如上，这种`（类型）`的用法就是 Java 中的类型转化语法，记住它，以后我们还会经常运用到
...
![img](https://style.youkeda.com/img/ham/course/j1/j1-3/camel.png?x-oss-process=image/resize,w_800/watermark,image_d2F0ZXJtYXNrLnBuZz94LW9zcy1wcm9jZXNzPWltYWdlL3Jlc2l6ZSx3XzEwMA==,t_60,g_se,x_10,y_10)
...
![img](https://style.youkeda.com/img/ham/course/j1/j1-3/method1.svg)
...
这种语法的详细解释可以 [点此阅读文档](https://ham.youkeda.com/articles/detail/5f3757005e205f30b2c2b140)
...
> 注意一下：charAt 方法返回的数据类型是 char，你可以理解称单个字符类型就是 char，它只能存储一个字符
...
> 这里我们可以看到有些用 getValue()，有些没有，[拓展阅读](https://ham.youkeda.com/articles/detail/5f37576b5e205f30b2c2b170)
>
...
![img](https://style.youkeda.com/img/ham/course/j1/j1-8/%E6%88%B7%E5%9E%8B%E5%9B%BE.png?x-oss-process=image/resize,w_800/watermark,image_d2F0ZXJtYXNrLnBuZz94LW9zcy1wcm9jZXNzPWltYWdlL3Jlc2l6ZSx3XzEwMA==,t_60,g_se,x_10,y_10)
...
在 Java 中**对象**如上述的户型图一样，把房子设计成一个户型图就是**抽象**的过程。![img](https://style.youkeda.com/img/ham/course/j1/j1-8/hourse-design.svg)
...
![img](https://style.youkeda.com/img/ham/course/j1/j1-8/createJava.png?x-oss-process=image/resize,w_800/watermark,image_d2F0ZXJtYXNrLnBuZz94LW9zcy1wcm9jZXNzPWltYWdlL3Jlc2l6ZSx3XzEwMA==,t_60,g_se,x_10,y_10)

点击新建类后，会出现对应的文件名输入框，输入 Java 文件名称就可以啦

![img](https://style.youkeda.com/img/ham/course/j1/j1-8/newJavaFile.png?x-oss-process=image/resize,w_800/watermark,image_d2F0ZXJtYXNrLnBuZz94LW9zcy1wcm9jZXNzPWltYWdlL3Jlc2l6ZSx3XzEwMA==,t_60,g_se,x_10,y_10)
...
自定义包其实就是创建文件夹，包路径就是文件夹相对路径，比如说 `com.youkeda.test` 这是一个包路径（注意包路径和子包路径直接使用`.`来做分隔），对应的文件路径就是 `src/main/java/com/youkeda/test`,如果工程没有这个文件夹，就需要自己创建一下和创建文件一样，只是要大家多创建几个子文件夹哦。

![img](https://style.youkeda.com/img/ham/course/j1/j1-8/package.png?x-oss-process=image/resize,w_800/watermark,image_d2F0ZXJtYXNrLnBuZz94LW9zcy1wcm9jZXNzPWltYWdlL3Jlc2l6ZSx3XzEwMA==,t_60,g_se,x_10,y_10)

如果我们想创建 `com.youkeda` 这个包，你可以一起输入

![img](https://style.youkeda.com/img/ham/course/j1/j1-8/newpackage.png?x-oss-process=image/resize,w_800/watermark,image_d2F0ZXJtYXNrLnBuZz94LW9zcy1wcm9jZXNzPWltYWdlL3Jlc2l6ZSx3XzEwMA==,t_60,g_se,x_10,y_10)

> 现在我们说一下包名的规则，一般来说包名的开始是一个公司或者一个组织的域名的反写，如果是个人也可以遵守类似的规则，举个例子，优课达的域名是 `youkeda.com`,那么包名的起始就是`com.youkeda`,我们就需要创建`com/youkeda`这样的两级目录。Java 的规范建议是用包管理代码

如果我们在某个包下创建 Java 文件，那么 Java 类的内容就需要多个一个声明包路径的语法

比如`com.youkeda.test.Hello.java` 这样的 Java 类，我们可以分析一下，它的包名是
...
![img](https://style.youkeda.com/img/ham/course/j1/j1-8/hourse-design.svg)
...
![img](https://style.youkeda.com/img/ham/course/j1/j1-8/hourse-my.svg)
...
![img](https://style.youkeda.com/img/ham/course/j2/java-map1.png?x-oss-process=image/resize,w_800/watermark,image_d2F0ZXJtYXNrLnBuZz94LW9zcy1wcm9jZXNzPWltYWdlL3Jlc2l6ZSx3XzEwMA==,t_60,g_se,x_10,y_10)
...
Map 的运用相对复杂一点，大家如果不是特别理解也没有关系，我们多练习几次，先习惯它的语法。我们通过解决实际场景的问题来逐步消化知识

> 完整的 Map 接口方法可以 [点击查看文档](https://ham.youkeda.com/articles/detail/5f3758fe5e205f30b2c2b2f0)
...
> 项目实战是一个融会贯通的过程，所以需要大家反复查看之前的知识内容从而完成任务哦，挑战一下自己，加油！！！
...
本项目用到了 Java 面向对象和 Java 网络知识，这在后面的课程里会继续.
